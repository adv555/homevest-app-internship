...
import { BaseAlertProps } from './types/base-alert-props.interface'
...
const Alert: React.FC<BaseAlertProps> = ({
  ...
  return (
    <div className={`flex p-4 mt-4 mb-1 text-sm rounded-lg bg-${type === AlertType.DANGER ? "red" : "green"} bg-opacity-60 rounded-lg"`} role="alert">
      ...
      <div>
        <span className="font-medium pt-6">{title}</span> {message} 
      </div>
    </div>  
  )
}
